...
func main (){
	...
	l := log.New(os.Stdout, "productRestApi", log.LstdFlags)
	v := data.NewValidation("localhost:9092")

	

	conn, err := grpc.Dial("localhost:9092",opts.WithInsecure())
	if err != nil {
		panic(err)
	}
	defer conn.Close()
...
c := make(chan os.Signal, 1) // buffer for one item
